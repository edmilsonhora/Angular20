...
namespace MyAngular20.AppWebAngular.Controllers
{
    ...
    public class NotasController : ControllerBase
    {
        ...
        public NotasController(IFacade facade)
        {
            this._facade = facade;
        }
        ...
        public IActionResult ObterTodos()
        {
            try
            {
                return Ok(_facade.Notas.ObterTodos());
            }
            catch (Exception ex)
            {
                _facade.Roolback();
        ...
        public IActionResult ObterPaginado(int pageIndex, int pageSize)
        {
            try
            {
                ...
                return Ok(_facade.Notas.ObterPaginado(pageIndex, pageSize));
            }
            catch (Exception ex)
            {
                _facade.Roolback();
        ...
        public IActionResult ObterPor(int id)
        {
            try
            {
                return Ok(_facade.Notas.ObterPor(id));
            }
            catch (Exception ex)
            {
                _facade.Roolback();
        ...
        public IActionResult Salvar(NotaView view)
        {
            try
            {
                _facade.Notas.Salvar(view);
                _facade.SaveChanges();
                return Ok(true);
            }
            catch (Exception ex)
